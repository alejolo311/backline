...
export function FestivalHeader({ festival }: { festival: Festival }) {
  ...
  return (
    <div className="border-b border-gray-200 bg-white px-4 py-3 flex items-center gap-3 flex-wrap">
      <div
        className="w-2.5 h-2.5 rounded-full flex-shrink-0"
        style={{ backgroundColor: festival.coverColor }}
      />
      <div className="flex-1 min-w-0">
        <h1 className="font-serif text-base font-normal truncate leading-tight">
          {festival.name}
        </h1>
      ...
      <div className="flex items-center gap-1">
        <Link
          ...
          target="_blank"
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5 rounded hover:bg-gray-100 transition-colors"
        ...
        <Link
          href={`/festival/${festival.id}/team`}
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5 rounded hover:bg-gray-100 transition-colors"
        >
          <Users size={13} />
        ...
        <Link
          href={`/festival/${festival.id}/settings`}
          className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5 rounded hover:bg-gray-100 transition-colors"
        >
          <Settings size={13} />
